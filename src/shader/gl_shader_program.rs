...
use super::ShaderComponent;


pub struct GLShaderProgram { // what will actually be used as a shader
    id: GLuint,
    _name: String,
...
impl GLShaderProgram {
    pub fn load_shader_program(filepath: &str, identifying_string: &str, geometry_included: bool) -> Result<GLShaderProgram, RenderError> { //returns the finalised shader struct
        ...
        let vert_shader = match ShaderComponent::load_and_compile_shader(&vert_shader_source, gl::VERTEX_SHADER) {
            Ok(shader) => shader,
        ...
        let frag_shader = match ShaderComponent::load_and_compile_shader(&frag_shader_source, gl::FRAGMENT_SHADER){
            Ok(shader) => shader,
        ...
        let geometry_shader = if geometry_included {
            ...
            let geometry_shader = match ShaderComponent::load_and_compile_shader(&geometry_shader_source, gl::FRAGMENT_SHADER){
                Ok(shader) => shader,
